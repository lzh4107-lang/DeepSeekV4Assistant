...
class HomePageController extends ChangeNotifier {
  ...
  StreamSubscription<String>? _chatTextInsertSub;
  ...
  void _setupDesktopFeatures() {
    ...
    _chatTextInsertSub = ChatTextInsertBus.instance.stream.listen((text) {
      _insertExternalText(text);
    });
  }

  void _insertExternalText(String value) {
    final insertion = value.trim();
    if (insertion.isEmpty || !_context.mounted) return;
    ...
    final separator = text.isEmpty || start == 0 || text[start - 1] == '\n'
        ? ''
        : '\n\n';
    final inserted = '$separator$insertion';
